...
class FlappyBird():
    ...
    # Gets an input from base and sets the birds position
    def move_bird(self):
        old_position = self.get_bird_pos()
        new_position = self.base.convertInput(self.base.getInput())
        if not self.move_detection(old_position, new_position)
            self.set_bird_pos(new_position)
